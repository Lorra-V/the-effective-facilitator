...
function normalizeOptions(raw: unknown): ScenarioOption[] {
  if (Array.isArray(raw)) return raw as ScenarioOption[];
  if (typeof raw === "string") {
    try {
      const parsed = JSON.parse(raw) as unknown;
      return Array.isArray(parsed) ? (parsed as ScenarioOption[]) : [];
    } catch {
      return [];
    }
  }
...
export default async function DisciplinePage({ params }: PageProps) {
  ...
  const pathLabel = path.title.includes("—")
    ? path.title.slice(0, path.title.indexOf("—")).trim()
    : path.title;

  return (
    <div className="tef-main-inner">
      <p style={{ color: "var(--tef-muted)", marginBottom: "0.35rem" }}>{pathLabel}</p>
      <h1 style={{ margin: 0 }}>{discipline.title}</h1>
      {discipline.subtitle ? (
        <p className="tef-subtitle" style={{ color: "var(--tef-muted)", marginTop: "0.5rem" }}>
          {discipline.subtitle}
        </p>
      ) : null}
